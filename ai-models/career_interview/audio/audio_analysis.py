...
def analyse_audio(audio_path: str, transcript: str = "") -> Dict:
    ...
    try:
        ...
        # ── Convert to WAV if needed (librosa handles wav best) ────────
        ...
        _logging.getLogger("audio_analysis").error(f"analyse_audio failed: {e}", exc_info=True)
        base["error"] = str(e)
        return base
...
def interpret_audio_metrics(metrics: Dict) -> Dict:
    ...
    if metrics["speech_ratio"] > 0.75: engagement_score += 0.5
    if metrics["energy_variance"] > 0.0005: engagement_score += 0.5
    if metrics["pitch_variance"] > 300: engagement_score += 0.5
    if filler_rate > 8: engagement_score -= 1.5
    if pause_rate > 7: engagement_score -= 1.0
    engagement_score = round(min(max(engagement_score, 1), 10), 1)

    # Hesitation score (inverse of fluency signals)
    hesitation_score = 0
    if pause_rate > 7: hesitation_score += 2
    elif pause_rate > 4: hesitation_score += 1
    if filler_rate > 8: hesitation_score += 2
    elif filler_rate > 4: hesitation_score += 1
    if metrics["avg_pause_duration"] > 2.0: hesitation_score += 1

    summary = (
        f"Pace: {pace:.0f} WPM | Fillers: {metrics['filler_word_count']} | "
        f"Pauses: {metrics['pause_count']} | Speech ratio: {metrics['speech_ratio']:.0%} | "
        f"Engagement: {engagement_score}/10 | Hesitation level: {hesitation_score}/5"
