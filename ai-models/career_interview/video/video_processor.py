...
def extract_audio_from_video(
    ...
    # Auto-create a temp file if no output path given
    if audio_output_path is None:
        tmp = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
        tmp.close()
        wav_output_path = tmp.name
    else:
    ...
    # ── Extract ────────────────────────────────────────────────────────────
    # -acodec pcm_s16le is critical for browser webm/opus — without it ffmpeg
    # may produce a WAV that passes validation but Whisper reads as silence.
    result = subprocess.run(
        [
            ...
            "-acodec", "pcm_s16le",       # explicit PCM — fixes browser webm/opus
            wav_output_path,
        ],
        capture_output=True,
        text=True,
