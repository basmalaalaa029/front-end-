"""
matching/engine.py — Identity-Aware Recruitment Engine

Score breakdown (100 pts):
  35 pts semantic similarity
  25 pts weighted skill match
  20 pts seniority + experience alignment
  15 pts role/title relevance
...
_model = None
_cache: dict = {}
_CACHE_FILE = "/tmp/job_matcher_embed_cache.npz"
...
def _get_model():
    ...
    try:
        ...
        shared = getattr(_vs, "_st_model", None)
        if shared is not None and shared != "unavailable":
            _model = shared
            return _model
    except Exception:
        pass
...
def _detect_intent(profile: dict) -> dict:
    ...
    return {
        ...
        "research_oriented": any(w in text for w in ["research", "phd", "thesis"]),
...
def pre_filter(profile: dict, jobs: list) -> list:
    """Hard remove clearly incompatible jobs before expensive scoring."""
...
def score_job(profile: dict, job: dict) -> dict:
    ...
    if len(shared_a) >= 4:
        sem_score = max(sem_score, 22)
    elif len(shared_a) >= 3:
        sem_score = max(sem_score, 17)
    elif len(shared_a) >= 2:
        sem_score = max(sem_score, 12)
    elif len(shared_a) >= 1 and sem_raw > 0.2:
        sem_score = max(sem_score, 8)
    ...
    if len(m_a) >= 3:
        skill_score = max(skill_score, 14)
    elif len(m_a) >= 2:
        skill_score = max(skill_score, 10)
    elif len(m_a) >= 1:
        skill_score = max(skill_score, 6)
    if fc <= 0.20:
        skill_score = int(skill_score * 0.5)
    ...
    if cv_sen == "junior":
        ...
        if t_senior and not t_junior:
            base_sn = 13
        elif j_junior and not j_senior:
            base_sn = 12
        else:
            base_sn = 20
    else:
        if t_junior and not t_senior:
            base_sn = 7
        elif j_senior or j_neutral:
            base_sn = 20
        else:
            base_sn = 14
    ...
    if title_score < 5 and fc >= 0.85:
        title_score = max(title_score, 6)
    elif title_score < 3 and fc >= 0.70:
        title_score = max(title_score, 3)

    if fc < 0.40:
        title_score = int(title_score * 0.40)
    elif fc < 0.70:
        title_score = int(title_score * 0.75)

    bonus = 0

    if intent.get("seeking_internship") and j_junior:
        bonus += 2
    ...
    if intent.get("finance_focus") and any(
            w in jt_lower for w in ["finance", "accounting", "banking", "financial"]):
        bonus += 2
    ...
    return {
        ...
        "score_extra": extra,
        "semantic_pct": round(sem_raw * 100, 1),
        "confidence": conf,
        "req_years": ry,
        "family_compat": round(fc, 2),
        "cv_family": cv_fam,
        "job_family": job_fam,
        "matched_skills": matched_disp[:8],
        ...
        "geo_flag": job.get("geo_flag", False),
        "tags": job.get("tags", []),
